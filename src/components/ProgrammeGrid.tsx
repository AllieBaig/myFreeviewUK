...
import { ChevronLeft, ChevronRight, Star, Info, Play, Trophy, Clock, Tv } from 'lucide-react';
...
export const ProgrammeGrid: React.FC<ProgrammeGridProps> = ({ 
  ...
  const renderGroupHeader = (title: string, icon?: React.ReactNode) => (
    <div className={cn(
      "flex items-center gap-3 mb-6 mt-8 first:mt-0",
      theme === 'dark' ? "text-white/40" : "text-black/40"
    )}>
      {icon}
      <h2 className="text-xs font-black uppercase tracking-[0.3em]">{title}</h2>
      <div className={cn("flex-grow h-[1px]", theme === 'dark' ? "bg-white/5" : "bg-black/5")} />
    </div>
  );
  ...
  return (
    <div className={cn(
      ...
      <div className={cn(
        ...
        {category === 'All' ? (
          <>
            {renderGroupHeader("Dynamic Channels", <Tv size={14} />)}
            <div className="mb-12">
              {CHANNELS.map(channel => renderChannel(channel))}
            </div>

            {renderGroupHeader("Explore by Category", <Trophy size={14} />)}
            {genres.map(genre => {
              ...
              return (
                <div key={genre} className="mb-8">
                  <h3 className={cn(
                    "text-[10px] font-black uppercase tracking-widest mb-4 px-4",
                    theme === 'dark' ? "text-blue-400/60" : "text-blue-600/60"
                  )}>{genre}</h3>
                  <div className="grid grid-cols-1 gap-2">
                    {genreChannels.map(channel => renderChannel(channel))}
                  </div>
                </div>
              );
            })}
